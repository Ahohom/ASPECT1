...
class ProfileScreen(MDScreen):
    ...
    def logout(self, *args):
        try:
            ...
            # Получаем экземпляр приложения и сбрасываем текущего пользователя
            app = MDApp.get_running_app()
            if hasattr(app, 'current_user'):
                app.current_user = None

            # Обновляем экраны, которые зависят от пользователя
            if hasattr(app, 'update_user_dependent_screens'):
...
ООО'АСПЕКТ'все права защищены© 2025г.
        ...
        content = MDLabel(
            ...
            size_hint_y=None,
            height=dp(2100),  # Примерная высота для всего текста
            halign="left",
            valign="top",
            padding=(20, 30),
            font_size="12sp"
