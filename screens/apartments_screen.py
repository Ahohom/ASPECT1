...
class ApartmentDescriptionScreen(MDScreen):
    ...
    def build_ui(self):
        ...
        self.scroll_view.add_widget(self.info_card)
    ...
    def _update_info(self, apartment_data):
        ...
        # 2. Категория и цена
        category_price_row = MDBoxLayout(size_hint_y=None, height=dp(30), spacing=dp(20))
        ...
        # 3. Город и адрес
        city_address_text = f"{apartment_data[2]}, {apartment_data[1]}" if apartment_data[2] and apartment_data[1] else "Адрес не указан"
        self.info_content.add_widget(MDLabel(
        ...
        # 4. Площадь и комнаты
        area_rooms_text = f"Площадь: {apartment_data[4]} м², комнат: {apartment_data[5]}" if apartment_data[4] and apartment_data[5] else "Площадь и комнаты не указаны"
        self.info_content.add_widget(MDLabel(
        ...
        self.info_content.add_widget(desc_label)

        # Обновляем высоту контента
        self.info_content.height = sum(child.height for child in self.info_content.children) + dp(25)
...
class ApartmentsScreen(MDScreen):
    ...
    def build_ui(self):
        ...
        search_container = MDBoxLayout(
            ...
            size_hint=(0.8, None),
            height=dp(36),  # Фиксированная высота контейнера
            pos_hint={'center_x': 0.56, 'top': 0.96},
