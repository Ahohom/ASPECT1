...
class AspectApp(MDApp):
    ...
    def initialize_database(self):
        db_path = get_db_path()
        Logger.info(f"Database: Initializing at {db_path}")

        try:
            # Простая проверка, что файл является SQLite базой
            conn = sqlite3.connect(db_path)
            c = conn.cursor()
            c.execute("SELECT name FROM sqlite_master WHERE type='table'")
            tables = c.fetchall()
            Logger.info(f"Database: Found tables: {tables}")
            conn.close()
        except sqlite3.Error as e:
            Logger.error(f"Database: Invalid or corrupted database: {str(e)}")
            if platform == 'android':
                # Попробуем перекопировать из assets
                try:
                    os.remove(db_path)
                    get_db_path()  # Это вызовет повторное копирование
                except Exception as e:
                    Logger.error(f"Database: Failed to recover: {str(e)}")
